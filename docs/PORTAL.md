...
| `/radio` | **Radio.** What plays on the watch now, with its controls and the volume; the nine keys of the Radio app (drop a station on one, drag one onto another to swap them, play, clear); **My list**, kept on the card as `radio/library.json` through the generic file API; a search in radio-browser.info (from the browser: the firmware learns nothing about the directory) with "only MP3" on by default; an address by hand; and the covers switch. A station's logo is drawn in the browser on a 160 px square and saved as `radio/logoN.jpg`. See [RADIO.md](RADIO.md). |
...
GET  /api/list?dir=  POST /api/upload  GET /api/download  POST /api/delete  POST /api/mkdir
                                 dir is one of apps photos music videos recordings redes lang icons radio,
...
GET  /api/radio   POST /api/radio   the Radio app's nine keys and what plays; do=set&k=N&name=&url=
                                 | clear&k= | swap&a=&b= | play&k= | test&name=&url= | pause | resume
                                 | stop | next | prev | vol&v= | art&on=  (docs/RADIO.md)
