...
It compiles the real `aos_ui`, `aos_apps` and all 23 dynamic apps (with
...
368x448 screen, text overflowing its container, and **clickable objects out
of the touch's reach** (fewer than 20 px between y = 24 and y = 410, the
landing rows of `aos_ui_touch_map`, unless the object contains one of those
rows) — the last one being invisible in the simulator, where the mouse
reaches everywhere.
