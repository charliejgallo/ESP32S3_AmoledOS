 ...
 *  @endcode
 */
#define LV_FONT_CUSTOM_DECLARE   LV_FONT_DECLARE(aos_montserrat_14)

/** Always set a default font */
#define LV_FONT_DEFAULT &aos_montserrat_14
...
#define LV_USE_FS_MEMFS 1   /* the Video app draws JPEG frames from memory (branch video) */
#if LV_USE_FS_MEMFS
...
#define LV_USE_SNAPSHOT 1   /* same as the firmware: aos_ui.c uses it for /api/captura */
...
#define LV_USE_SDL              1 
    ...
    /* PARTIAL on purpose, even though LVGL recommends DIRECT for performance:
     * it is the mode the board runs in. In DIRECT the buffer is full-screen
     * and a missing repaint does NOT stay stuck, so that whole class of bug is
     * invisible in the simulator by construction. In partial mode the
     * simulator can prove you wrong, which is what it is for. See the trap "a
     * COLOURED background shows transfer errors" in docs/HANDOFF-APPS.md. */
