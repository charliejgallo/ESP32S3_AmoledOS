 ...
 */
/*
 * AmoledOS: las secciones se copiaban pegadas una tras otra, sumando tamanos
 * crudos. Si .data o .rodata no median un multiplo de 4, la seccion siguiente
 * quedaba desalineada; y .data.rel.ro es justamente una tabla de punteros, asi
 * que cada lectura salia torcida y la app moria con LoadProhibited leyendo
 * direcciones basura. Medido: claudito quedaba corrida 3 bytes, g2043 tambien
 * (su .data mide 1 byte con alineamiento 1). Alineamos a 8, que cubre tanto los
 * punteros de 4 como los .bss con alineamiento 8.
 */
#define ELF_SEC_ALIGN       8
#define ELF_SEC_ALIGN_PTR(p) \
    ((uint8_t *)(((uintptr_t)(p) + (ELF_SEC_ALIGN - 1)) & ~(uintptr_t)(ELF_SEC_ALIGN - 1)))
...
static int esp_elf_load_section(esp_elf_t *elf, const uint8_t *pbuf)
{
    ...
    for (uint32_t i = 0; i < ehdr->shnum; i++) {
        ...
        if (stype(&shdr[i], SHT_PROGBITS) && sflags(&shdr[i], SHF_ALLOC)) {
            if (sflags(&shdr[i], SHF_EXECINSTR) && !strcmp(ELF_TEXT, name)) {
                ...
                /* AmoledOS 2026-09-05: el tamano se guarda SIN redondear.
                 *
                 * Aca decia ELF_ALIGN(shdr[i].size, 4), y ese redondeo entra
                 * despues en la comparacion de esp_elf_map_sym(), que traduce
                 * direcciones de enlace a direcciones reales preguntando
                 * "cae en [v_addr, v_addr + size)". Si .text no mide un
                 * multiplo de 4 y .rodata arranca pegada -cosa que pasa
                 * cuando .rodata tiene alineamiento 1-, el rango de .text
                 * termina incluyendo el primer byte de .rodata. Como .text se
                 * compara primero, TODO puntero a .rodata se traduce al
                 * buffer de codigo: las cadenas, las tablas de constantes,
                 * todo. La app carga, registra bien (su init() solo copia
                 * punteros) y al abrirla queda la pantalla en negro.
                 *
                 * Estuvo latente meses: con los .so compilados en -O0 ninguna
                 * de las 19 apps caia en el caso. Al pasar a -Os cambiaron
                 * los tamanos y cayeron 8 de 19 (afinador, cotiz, dados,
                 * flappy, hello_app, laberinto, recorder y sensores), lo que
                 * parecia un problema del optimizador y no lo era.
                 *
                 * El redondeo que si hace falta es el de la RESERVA y el de
                 * la copia, no el del rango: van explicitos mas abajo. */
    ...
    /* La reserva y la copia SI van redondeadas a 4: el destino tiene que
     * quedar alineado y copiar de mas unos bytes del buffer del archivo es
     * inofensivo. Lo que no puede redondearse es el tamano guardado, que se
     * usa para decidir a que seccion pertenece una direccion. */
    ...
    /* AmoledOS: +3 huecos de alineacion, uno por cada seccion que ahora se
     * redondea hacia arriba antes de ubicarla (ver ELF_SEC_ALIGN_PTR). */
        ...
        if (stype(&shdr[i], SHT_RELA)) {
            ...
            for (int i = 0; i < nr_reloc; i++) {
                ...
                if (type == STT_COMMON || type == STT_OBJECT || type == STT_SECTION) {
...
#if CONFIG_ELF_LOADER_BUS_ADDRESS_MIRROR
                            /* AmoledOS: esto daba por sentado que todo simbolo
                             * global vive en ".data" y le restaba esa base. Un
                             * array global const de punteros vive en
                             * ".data.rel.ro", asi que su entrada en la GOT
                             * quedaba apuntando cientos de bytes antes del
                             * bloque de datos -> LoadProhibited leyendo basura.
                             * esp_elf_map_sym() ubica la seccion correcta por
                             * rango de direccion virtual. */
                    ...
                    if (!addr && sym->shndx != SHN_UNDEF) {
#if CONFIG_ELF_LOADER_BUS_ADDRESS_MIRROR
                        /* AmoledOS: mismo problema que arriba, aca dando por
                         * sentado ".text". */
                        addr = esp_elf_map_sym(elf, sym->value);
#else
                        addr = (uintptr_t)(elf->psegment + sym->value - elf->svaddr);
