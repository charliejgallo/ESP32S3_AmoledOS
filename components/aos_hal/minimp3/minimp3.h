...
int mp3dec_decode_frame(mp3dec_t *dec, const uint8_t *mp3, int mp3_bytes, mp3d_sample_t *pcm, mp3dec_frame_info_t *info)
{
    ...
    /* AmoledOS: the scratch (~16 KB) comes from the caller instead of the
     * stack, so the decoding task can keep a small stack in internal RAM. */
    mp3dec_scratch_t *scratch = MINIMP3_SCRATCH;
#else
    mp3dec_scratch_t scratch_on_stack, *scratch = &scratch_on_stack;
#endif
