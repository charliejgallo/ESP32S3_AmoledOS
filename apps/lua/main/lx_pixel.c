/*
 * LX_PIXEL - pixel art engine (see lx_pixel.h)
 *
...
static inline int lip_floor(const lx_buf_t *b, int x)
{
    int i = x - b->lip_x0;
    return (unsigned)i < (unsigned)b->lip_n ? b->lip[i] : b->lip_def;
}

static inline bool visible(const lx_buf_t *b, int x, int y)
{
...
{
    return f < 0 ? lx_mix(c, 0x0000, -f) : lx_mix(c, 0xFFFF, f);
}
...
#define LX_JOIN_SLACK   380

static inline int rect_area(const lx_rect_t *r)
{
    return (r->x1 - r->x0) * (r->y1 - r->y0);
}

static int join_cost(const lx_rect_t *a, const lx_rect_t *b)
{
...
static void join_into(lx_rect_t *a, const lx_rect_t *b)
{
...
void lx_dirty_add(lx_dirty_t *d, int x, int y, int w, int h)
{
    ...
    n.x1 = (int16_t)(x + w > LX_W ? LX_W : x + w);
    n.y1 = (int16_t)(y + h > LX_H ? LX_H : y + h);
    ...
    if (best >= 0 && (best_cost <= LX_JOIN_SLACK || d->n >= LX_MAX_DIRTY)) {
...
void lx_expand(const uint16_t *src, uint16_t *dst, const lx_rect_t *r)
{
    ...
    for (int y = r->y0; y < r->y1; y++) {
        const uint16_t *s   = src + (size_t)y * LX_W + r->x0;
        uint16_t       *row = dst + (size_t)y * LX_SCALE * dw + r->x0 * LX_SCALE;
        ...
        memcpy(row + dw, row, (size_t)span * LX_SCALE * sizeof(uint16_t));
    }
}
...
void lx_line(lx_buf_t *b, int x0, int y0, int x1, int y1, uint16_t c)
{
    ...
    for (int i = 0; i <= steps; i++) {
        lx_px(b, x0 + dx * i / steps, y0 + dy * i / steps, c);
    }
}
...
void lx_disc(lx_buf_t *b, int cx, int cy, int r, uint16_t c)
{
    ...
    for (int dy = -r; dy <= r; dy++) {
        ...
        lx_hline(b, cx - span, cy + dy, span * 2 + 1, c);
    }
}
...
void lx_ring(lx_buf_t *b, int cx, int cy, int r, uint16_t c)
{
    ...
    int x = r, y = 0, err = 1 - r;
    while (x >= y) {
        lx_px(b, cx + x, cy + y, c); lx_px(b, cx + y, cy + x, c);
        lx_px(b, cx - y, cy + x, c); lx_px(b, cx - x, cy + y, c);
        lx_px(b, cx - x, cy - y, c); lx_px(b, cx - y, cy - x, c);
        lx_px(b, cx + y, cy - x, c); lx_px(b, cx + x, cy - y, c);
        y++;
        if (err < 0) {
...
void lx_ellipse(lx_buf_t *b, int cx, int cy, int rx, int ry, uint16_t c)
{
    ...
    for (int dy = -ry; dy <= ry; dy++) {
        int s = lx_isqrt((ry * ry - dy * dy) * 1024 + ry * 32);
        int span = rx * s / (ry * 32);
        lx_hline(b, cx - span, cy + dy, span * 2 + 1, c);
    }
}
...
void lx_round(lx_buf_t *b, int x, int y, int w, int h, int cut, uint16_t c)
{
    for (int yy = 0; yy < h; yy++) {
        ...
        lx_hline(b, x + inset, y + yy, w - inset * 2, c);
    }
}
...
void lx_shade(lx_buf_t *b, int x, int y, int w, int h, int f)
{
    ...
    for (int yy = y; yy < y + h; yy++) {
        for (int xx = x; xx < x + w; xx++) {
            lx_px_mix(b, xx, yy, target, amount);
        }
    }
}
...
void lx_glow(lx_buf_t *b, int cx, int cy, int r, uint16_t c, int f)
{
    ...
    for (int dy = -r; dy <= r; dy++) {
        ...
        for (int dx = -span; dx <= span; dx++) {
            int d = dx * dx + dy * dy;
            lx_px_mix(b, cx + dx, cy + dy, c, f - ((d * inv) >> 12));
        }
    }
}
...
static const uint8_t lx_sin_q[65] = {
...
{
    brad &= 0xFF;
    if (brad <= 64)  return  lx_sin_q[brad];
    if (brad <= 128) return  lx_sin_q[128 - brad];
    if (brad <= 192) return -lx_sin_q[brad - 128];
    return -lx_sin_q[256 - brad];
}

int lx_cos(int brad)
{
    return lx_sin(brad + 64);
}
...
#define LX_FONT_FIRST   32
#define LX_FONT_LAST    95

static const uint8_t lx_font5x7[LX_FONT_LAST - LX_FONT_FIRST + 1][LX_CH_W] = {
...
{
    int n = (int)strlen(s);
    return n > 0 ? (n * LX_CH_ADV - 1) * scale : 0;
}

void lx_text(lx_buf_t *b, int x, int y, const char *s, uint16_t c, int scale)
{
    for (; *s; s++, x += LX_CH_ADV * scale) {
        uint8_t ch = (uint8_t)*s;
        if (ch >= 'a' && ch <= 'z') {
            ch = (uint8_t)(ch - 32);        /* the font is upper case only */
        }
        if (ch < LX_FONT_FIRST || ch > LX_FONT_LAST) {
            continue;
        }
        const uint8_t *cols = lx_font5x7[ch - LX_FONT_FIRST];
        for (int cx = 0; cx < LX_CH_W; cx++) {
            uint8_t bits = cols[cx];
            for (int cy = 0; cy < LX_CH_H; cy++) {
                if (bits & (1u << cy)) {
                    lx_rect(b, x + cx * scale, y + cy * scale, scale, scale, c);
                }
            }
        }
    }
                ...
                uint16_t fill, uint16_t outline, int scale)
{
    ...
    for (int i = 0; i < 8; i++) {
        lx_text(b, x + off[i][0], y + off[i][1], s, outline, scale);
    }
    /* and a drop shadow one more pixel down, so it reads on bright grass */
    lx_text(b, x, y + 2, s, outline, scale);
    lx_text(b, x, y, s, fill, scale);
}
