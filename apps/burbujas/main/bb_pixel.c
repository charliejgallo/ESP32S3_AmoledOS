/*
 * BURBUJAS - pixel art engine (see bb_pixel.h)
 *
...
static inline int lip_floor(const bb_buf_t *b, int x)
{
    int i = x - b->lip_x0;
    return (unsigned)i < (unsigned)b->lip_n ? b->lip[i] : b->lip_def;
}

static inline bool visible(const bb_buf_t *b, int x, int y)
{
...
{
    return f < 0 ? bb_mix(c, 0x0000, -f) : bb_mix(c, 0xFFFF, f);
}
...
#define BB_JOIN_SLACK   380

static inline int rect_area(const bb_rect_t *r)
{
    return (r->x1 - r->x0) * (r->y1 - r->y0);
}

static int join_cost(const bb_rect_t *a, const bb_rect_t *b)
{
...
static void join_into(bb_rect_t *a, const bb_rect_t *b)
{
...
void bb_dirty_add(bb_dirty_t *d, int x, int y, int w, int h)
{
    ...
    n.x1 = (int16_t)(x + w > BB_W ? BB_W : x + w);
    n.y1 = (int16_t)(y + h > BB_H ? BB_H : y + h);
    ...
    if (best >= 0 && (best_cost <= BB_JOIN_SLACK || d->n >= BB_MAX_DIRTY)) {
...
void bb_expand(const uint16_t *src, uint16_t *dst, const bb_rect_t *r)
{
    ...
    for (int y = r->y0; y < r->y1; y++) {
        const uint16_t *s   = src + (size_t)y * BB_W + r->x0;
        uint16_t       *row = dst + (size_t)y * BB_SCALE * dw + r->x0 * BB_SCALE;
        ...
        memcpy(row + dw, row, (size_t)span * BB_SCALE * sizeof(uint16_t));
    }
}
...
void bb_line(bb_buf_t *b, int x0, int y0, int x1, int y1, uint16_t c)
{
    ...
    for (int i = 0; i <= steps; i++) {
        bb_px(b, x0 + dx * i / steps, y0 + dy * i / steps, c);
    }
}
...
void bb_disc(bb_buf_t *b, int cx, int cy, int r, uint16_t c)
{
    ...
    for (int dy = -r; dy <= r; dy++) {
        ...
        bb_hline(b, cx - span, cy + dy, span * 2 + 1, c);
    }
}
...
void bb_ring(bb_buf_t *b, int cx, int cy, int r, uint16_t c)
{
    ...
    int x = r, y = 0, err = 1 - r;
    while (x >= y) {
        bb_px(b, cx + x, cy + y, c); bb_px(b, cx + y, cy + x, c);
        bb_px(b, cx - y, cy + x, c); bb_px(b, cx - x, cy + y, c);
        bb_px(b, cx - x, cy - y, c); bb_px(b, cx - y, cy - x, c);
        bb_px(b, cx + y, cy - x, c); bb_px(b, cx + x, cy - y, c);
        y++;
        if (err < 0) {
...
void bb_ellipse(bb_buf_t *b, int cx, int cy, int rx, int ry, uint16_t c)
{
    ...
    for (int dy = -ry; dy <= ry; dy++) {
        int s = bb_isqrt((ry * ry - dy * dy) * 1024 + ry * 32);
        int span = rx * s / (ry * 32);
        bb_hline(b, cx - span, cy + dy, span * 2 + 1, c);
    }
}
...
void bb_round(bb_buf_t *b, int x, int y, int w, int h, int cut, uint16_t c)
{
    for (int yy = 0; yy < h; yy++) {
        ...
        bb_hline(b, x + inset, y + yy, w - inset * 2, c);
    }
}
...
void bb_shade(bb_buf_t *b, int x, int y, int w, int h, int f)
{
    ...
    for (int yy = y; yy < y + h; yy++) {
        for (int xx = x; xx < x + w; xx++) {
            bb_px_mix(b, xx, yy, target, amount);
        }
    }
}
...
void bb_glow(bb_buf_t *b, int cx, int cy, int r, uint16_t c, int f)
{
    ...
    for (int dy = -r; dy <= r; dy++) {
        ...
        for (int dx = -span; dx <= span; dx++) {
            int d = dx * dx + dy * dy;
            bb_px_mix(b, cx + dx, cy + dy, c, f - ((d * inv) >> 12));
        }
    }
}
...
static const uint8_t bb_sin_q[65] = {
...
{
    brad &= 0xFF;
    if (brad <= 64)  return  bb_sin_q[brad];
    if (brad <= 128) return  bb_sin_q[128 - brad];
    if (brad <= 192) return -bb_sin_q[brad - 128];
    return -bb_sin_q[256 - brad];
}

int bb_cos(int brad)
{
    return bb_sin(brad + 64);
}
...
#define BB_FONT_FIRST   32
#define BB_FONT_LAST    95

static const uint8_t bb_font5x7[BB_FONT_LAST - BB_FONT_FIRST + 1][BB_CH_W] = {
...
{
    int n = (int)strlen(s);
    return n > 0 ? (n * BB_CH_ADV - 1) * scale : 0;
}

void bb_text(bb_buf_t *b, int x, int y, const char *s, uint16_t c, int scale)
{
    for (; *s; s++, x += BB_CH_ADV * scale) {
        uint8_t ch = (uint8_t)*s;
        if (ch >= 'a' && ch <= 'z') {
            ch = (uint8_t)(ch - 32);        /* the font is upper case only */
        }
        if (ch < BB_FONT_FIRST || ch > BB_FONT_LAST) {
            continue;
        }
        const uint8_t *cols = bb_font5x7[ch - BB_FONT_FIRST];
        for (int cx = 0; cx < BB_CH_W; cx++) {
            uint8_t bits = cols[cx];
            for (int cy = 0; cy < BB_CH_H; cy++) {
                if (bits & (1u << cy)) {
                    bb_rect(b, x + cx * scale, y + cy * scale, scale, scale, c);
                }
            }
        }
    }
                ...
                uint16_t fill, uint16_t outline, int scale)
{
    ...
    for (int i = 0; i < 8; i++) {
        bb_text(b, x + off[i][0], y + off[i][1], s, outline, scale);
    }
    /* and a drop shadow one more pixel down, so it reads on bright grass */
    bb_text(b, x, y + 2, s, outline, scale);
    bb_text(b, x, y, s, fill, scale);
}
