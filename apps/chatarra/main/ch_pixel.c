/*
 * CLAUDE JUMP - pixel art engine (see ch_pixel.h)
 *
 ...
 * ch_expand_n() upscales to any scale, for the shop's large preview.
 */
#include "ch_pixel.h"
...
void ch_dirty_add(ch_dirty_t *d, int x, int y, int w, int h)
{
    ...
    n.x1 = (int16_t)(x + w > CH_W ? CH_W : x + w);
    n.y1 = (int16_t)(y + h > CH_H ? CH_H : y + h);
...
void ch_expand(const uint16_t *src, uint16_t *dst, const ch_rect_t *r)
{
    ...
    for (int y = r->y0; y < r->y1; y++) {
        const uint16_t *s   = src + (size_t)y * CH_W + r->x0;
        uint16_t       *row = dst + (size_t)y * CH_SCALE * dw + r->x0 * CH_SCALE;
        ...
        memcpy(row + dw, row, (size_t)span * CH_SCALE * sizeof(uint16_t));
    }
}
 ...
 *
 * ch_expand() above is tied to CH_W and CH_SCALE because it is the hot path
...
void ch_line(ch_buf_t *b, int x0, int y0, int x1, int y1, uint16_t c)
{
    ...
    for (int i = 0; i <= steps; i++) {
        ch_px(b, x0 + dx * i / steps, y0 + dy * i / steps, c);
    }
}
...
void ch_disc(ch_buf_t *b, int cx, int cy, int r, uint16_t c)
{
    ...
    for (int dy = -r; dy <= r; dy++) {
        ...
        ch_hline(b, cx - span, cy + dy, span * 2 + 1, c);
    }
}
...
void ch_ring(ch_buf_t *b, int cx, int cy, int r, uint16_t c)
{
    ...
    int x = r, y = 0, err = 1 - r;
    while (x >= y) {
        ch_px(b, cx + x, cy + y, c); ch_px(b, cx + y, cy + x, c);
        ch_px(b, cx - y, cy + x, c); ch_px(b, cx - x, cy + y, c);
        ch_px(b, cx - x, cy - y, c); ch_px(b, cx - y, cy - x, c);
        ch_px(b, cx + y, cy - x, c); ch_px(b, cx + x, cy - y, c);
        y++;
        if (err < 0) {
...
void ch_round(ch_buf_t *b, int x, int y, int w, int h, int cut, uint16_t c)
{
    for (int yy = 0; yy < h; yy++) {
        ...
        ch_hline(b, x + inset, y + yy, w - inset * 2, c);
    }
}
...
void ch_vgrad(ch_buf_t *b, int x, int y0, int w, int y1, uint16_t top, uint16_t bot)
{
    ...
    for (int y = y0; y <= y1; y++) {
        ...
        ch_hline(b, x, y, w, ch_mix(top, bot, f));
    }
}
...
void ch_glow(ch_buf_t *b, int cx, int cy, int r, uint16_t c, int f)
{
    ...
    for (int dy = -r; dy <= r; dy++) {
        ...
        for (int xx = x0; xx <= x1; xx++) {
            ...
            row[xx] = ch_mix(row[xx], c, f - ((d * inv) >> 12));
        }
    }
}
 ...
 * indexed by the character. The forty-branch switch Claude Jump had cost
 * .text, and here .text is the scarce resource: the table lives in .rodata,
 * which the loader sends to PSRAM and costs nothing.
 *
...
void ch_text(ch_buf_t *b, int x, int y, const char *s, uint16_t c)
{
    for (; *s; s++, x += CH_FADV) {
        ...
        if (ch < CH_FONT_FIRST || ch > CH_FONT_LAST) {
            continue;
        }
