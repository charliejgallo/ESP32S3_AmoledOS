#!/usr/bin/env python3
"""Monster Hop - reference compositor.
...
def screen(x, y, z):
    ...
    return (PX_X[0] * x + PX_Y[0] * y, PX_X[1] * x + PX_Y[1] * y - FLOOR_PX * z / FLOOR_M)
...
class Canvas:
    ...
    def draw(self, spr, wx, wy, wz, palette=None, alpha=1.0, shadow=True):
        ...
        if shadow and spr.sh is not None:
            # the shadow darkens what lies at the ground plane under the sprite
            gz = spr.info.get('shadow_z', wz)
            dg = depth(wx, wy, wz + 0.0)
            a = spr.sh[sl] / 255.0 * 0.55
            near = np.abs(self.z[dst] - dg) < 40
            m = a * near
