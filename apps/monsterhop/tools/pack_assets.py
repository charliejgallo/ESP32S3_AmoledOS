#!/usr/bin/env python3
"""
MONSTER HOP - packs the Blender renders into monsterhop.pak, the one file
the watch reads (main/mh_art.c has the layout).

    python3 tools/pack_assets.py [--only dir,dir] [-v]   # -> assets/monsterhop.pak
...
become <base>_sh and <base>_gl. Palettes (palettes.json in a dir) become
pal_<name> blobs, levels (assets/levels/*.bin) lvl_<name> blobs.
"""
import json
import os
import re
...
BPP = {COL: 4, LID: 3, PLANE: 1, GLOW: 2, IMG: 3}
...
def main():
    ...
    r = subprocess.run([sys.executable, os.path.join(ROOT, 'tools', 'levels.py'), '--check'],
        ...
        for name, info in sorted(meta.items()):
            ...
            if 'levels' in info:
                ...
                blobs[base + '_spots'] = struct.pack('<H', len(pts)) + b''.join(
                    struct.pack('<hh', int(round(x)), int(round(y))) for x, y in pts)
            if 'sh' in f:
    ...
    for f in os.listdir(CARD):
        if f.startswith('monsterhop.pak'):
            os.remove(os.path.join(CARD, f))
    parts = [blob[i:i + PART] for i in range(0, len(blob), PART)]
    for i, part in enumerate(parts):
        with open(os.path.join(CARD, 'monsterhop.pak' + ('.%d' % i if i else '')), 'wb') as fh:
            fh.write(part)
