/*
 * MONSTER HOP - pixels (see mh_gfx.h)
 */
...
void mh_set_yield(void (*hook)(void)) { s_yield = hook; }
void mh_yield(void) { if (s_yield) s_yield(); }
void mh_set_clock(uint32_t (*clock)(void)) { s_clock = clock; }
uint32_t mh_clock(void) { return s_clock ? s_clock() : 0; }

uint32_t mh_mix(uint32_t a, uint32_t b, int t)
{
...
void mh_rect_blend(mh_img_t *im, int x, int y, int w, int h, uint16_t c, int alpha)
{
    ...
    for (int yy = y0; yy < y1; yy++) {
        uint16_t *row = im->px + (size_t)yy * im->w;
        for (int xx = x0; xx < x1; xx++) row[xx] = mh_blend(row[xx], c, alpha);
    }
}

static inline void px_blend(mh_img_t *im, int x, int y, uint16_t c, int a)
{
...
void mh_disc(mh_img_t *im, int cx16, int cy16, int r16, uint16_t c, int alpha)
{
    ...
    int x0 = mh_ifloor(cx - r - 1), x1 = mh_ifloor(cx + r + 2);
    int y0 = mh_ifloor(cy - r - 1), y1 = mh_ifloor(cy + r + 2);
...
void mh_shadow_ellipse(mh_img_t *im, int cx, int cy, int rx, int ry, int alpha)
{
    ...
    for (int y = y0; y <= y1; y++) {
        ...
        for (int x = x0; x <= x1; x++) {
            ...
            if (d >= 256) continue;
            int k = 256 - d;
            row[x] = mh_darken(row[x], 256 - (k * alpha >> 8));
        }
    }
}
...
void mh_mask_draw(mh_img_t *im, const mh_mask_t *m, int x, int y, uint16_t c, int alpha)
{
    ...
    for (int yy = y0; yy < y1; yy++) {
        ...
        for (int xx = x0; xx < x1; xx++) {
            int a = ma[xx];
            if (!a) continue;
            row[xx] = mh_blend(row[xx], c, alpha >= 255 ? a : a * alpha >> 8);
        }
    }
}
