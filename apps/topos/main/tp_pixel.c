...
void tp_expand(const uint16_t *src, uint16_t *dst, const tp_rect_t *r)
{
    ...
    for (int y = r->y0; y < r->y1; y++) {
        const uint16_t *s   = src + (size_t)y * TP_W + r->x0;
        uint16_t       *row = dst + (size_t)y * TP_SCALE * dw + r->x0 * TP_SCALE;
        ...
        memcpy(row + dw, row, (size_t)span * TP_SCALE * sizeof(uint16_t));
    }
}
