...
void M_SaveSelect(int choice)
{
    ...
    /* AmoledOS: there is no keyboard; an empty slot gets a name that one
     * more press of FIRE (Enter) confirms */
    if (savegamestrings[choice][0] == 0)
        M_snprintf(savegamestrings[choice], SAVESTRINGSIZE, "SLOT %d", choice + 1);
...
/* AmoledOS: what the pad's FIRE should mean right now (port/dg_input.c) */
int dg_menu_state(void)
{
    if (!menuactive) return 0;
    if (messageToPrint && messageNeedsInput) return 2;   /* a y/n question */
    return 1;
}
