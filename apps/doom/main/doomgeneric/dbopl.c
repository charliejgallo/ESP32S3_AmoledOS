...
/* AmoledOS: the tables and the rate set-up in float: the S3 has a single
 * precision FPU and the firmware lends no double arithmetic to the apps */
#include <math.h>
...
#define OPLRATE		((float)(14318180.0f / 288.0f))
...
//	float original = rate;
	...
	for ( i = 0; i < 16; i++ ) {
		self->freqMul[i] = (Bit32u)( 0.5f + freqScale * FreqCreateTable[ i ] );
	}
#else
	freqScale = (Bit32u)( 0.5f + scale * ( 1 << ( WAVE_SH - 1 - 10)));
	for ( i = 0; i < 16; i++ ) {
...
void DBOPL_InitTables( void ) {
	...
	for ( i = 0; i < 256; i++ ) {
		...
		ExpTable[i] = (int)( 0.5f + ( powf(2.0f, ( 255 - i) * ( 1.0f /256 ) )-1) * 1024 );
...
#endif
#if ( DBOPL_WAVE == WAVE_HANDLER )
	//Add 0.5f for the trunc rounding of the integer cast
	//Do a PI sinetable instead of the original 0.5f PI
	for ( i = 0; i < 512; i++ ) {
		SinTable[i] = (Bit16s)( 0.5f - log10f( sinf( (i + 0.5f) * (PI / 512.0f) ) ) / log10f(2.0f)*256 );
	}
#endif
#if ( DBOPL_WAVE == WAVE_TABLEMUL )
	...
	for ( i = 0; i < 384; i++ ) {
		...
		float val = ( 0.5f + ( powf(2.0f, -1.0f + ( 255 - s) * ( 1.0f /256 ) )) * ( 1 << MUL_SH ));
		MulTable[i] = (Bit16u)(val);
	}

	//Sine Wave Base
	for ( i = 0; i < 512; i++ ) {
		WaveTable[ 0x0200 + i ] = (Bit16s)(sinf( (i + 0.5f) * (PI / 512.0f) ) * 4084);
		WaveTable[ 0x0000 + i ] = -WaveTable[ 0x200 + i ];
	}
	//Exponential wave
	for ( i = 0; i < 256; i++ ) {
		WaveTable[ 0x700 + i ] = (Bit16s)( 0.5f + ( powf(2.0f, -1.0f + ( 255 - i * 8) * ( 1.0f /256 ) ) ) * 4085 );
	...
	//Sine Wave Base
	for ( i = 0; i < 512; i++ ) {
		WaveTable[ 0x0200 + i ] = (Bit16s)( 0.5f - log10f( sinf( (i + 0.5f) * (PI / 512.0f) ) ) / log10f(2.0f)*256 );
