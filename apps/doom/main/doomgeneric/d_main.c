...
int             show_endoom = 0;   /* AmoledOS: no text mode */
...
void doomgeneric_Tick()
{
    ...
    /* AmoledOS: only when a tic ran. TryRunTics returns after a tic's
     * worth of waiting even if none did, and drawing the same state again
     * is 100 % of a core for nothing (46 frames a second for 35 tics). */
    {
        static int last_drawn = -1;
        if (screenvisible && gametic != last_drawn)
        {
            last_drawn = gametic;
            D_Display ();
        }
