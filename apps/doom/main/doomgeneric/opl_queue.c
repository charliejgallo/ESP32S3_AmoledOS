...
/* AmoledOS: the engine's malloc, so the queue goes when the engine goes */
#include "dg_compat.h"
                               ...
                               uint64_t time, float factor)
{
    ...
    for (i = 0; i < queue->num_entries; ++i)
    {
        ...
        /* AmoledOS: in 32 bits. A pending callback is at most a few seconds
         * away, and float <-> 64-bit conversions are libgcc calls the
         * firmware does not lend */
        if (offset > 0x7fffffff) offset = 0x7fffffff;
        if (offset < -0x7fffffff) offset = -0x7fffffff;
        queue->entries[i].time = time + (int64_t) (int32_t) ((float) (int32_t) offset * factor);
    }
}
